    ...
    onCompleted: (res) => {
      ...
      localUser
        ? localStorage.setItem(
            ...
            JSON.stringify({
              ...localUser,
              avatarURL: res.updateUser.user.avatarURL,
            })
          )
        : null;
      setTimeout(() => {
      ...
      <div className="flex flex-col gap-20">
        <div className="self-center -overflow-hidden rounded-sm border border-stroke shadow-default dark:border-strokedark dark:bg-boxdark w-full max-w-7xl">
          ...
          <div className="px-4 pb-6 lg:pb-8 xl:pb-11.5 flex items-center gap-6 relative -mt-10 z-50">
            <UploadProfilePicture
              files={[{ preview: localUser?.avatarURL }]}
              isLoading={loading}
              setFiles={onProfileChange}
            />
            <div className="mt-4">
        ...
        <section className="flex flex-col gap-6">
          {localUser?.watchlist && localUser?.watchlist.length > 0 ? (
            ...
            <Link
              href="/browse"
              className="text-2xl font-bold text-center flex self-center items-center gap-2"
            >

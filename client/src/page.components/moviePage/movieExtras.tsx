    ...
    <section className="p-4 2xl:p-10 w-full 2xl:max-w-[75%] space-y-4">
      <MovieGrid
        ...
        renderItem={(crew: Cast) => (
          <div className="flex flex-col embla__slide gap-1" key={crew?.id}>
            ...
            {crew?.character && (
              <span className="text-slate-400 max-lg:max-w-40">as {crew?.character}</span>
            )}
          </div>
        )}
        columns={5}
      ...
      <MovieGrid
        ...
        renderItem={(movie: Movie) => (
            <Link className="embla__slide" href={`/movie/${movie?.id}`} key={movie?.id}>
            <div className="flex flex-col gap-1" key={movie?.id}>
              <img
                ...
                alt=""
              />
              <p className="font-light max-w-40 lg:max-w-sm">{movie?.title}</p>
            </div>
        </Link>
        )}
        columns={6}
